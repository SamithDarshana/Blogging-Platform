...
const NewBlog = () => {
  ...
  const handleSubmit = async (e) => { 
    e.preventDefault()
    try { 
      await axios.post(`http://localhost:4000/api/blog/addblog/${id}`, {title, content, tags}) 
      setPopup({ show: true, message: 'Blog post added successfully!', type: 'success' })
      setTitle('')
      setContent('')
      setTags('')
      setId('')
      ...
      console.log(error)
      setPopup({ show: true, message: 'Failed to add blog post.', type: 'error' })
    } finally {
  ...
  return (
    <div className="flex flex-col justify-center items-center p-5 pl-28 pr-28">
      <div className="w-full mx-auto mt-10 p-6 bg-slate-100 rounded-lg shadow-md flex flex-col items-center justify-center">
        <h1 className="text-teal-600 text-xl font-bold mb-4">Create New Blog Post</h1> 
        {popup.show && (
...
const Popup = ({ message, type }) => {
  return (
      <div className={`fixed top-4 right-4 p-4 rounded shadow-md z-50 ${type === 'success' ? 'bg-green-500 text-white' : 'bg-red-500 text-white'}`}>
          {message}
      </div>
  );
};
